...
|---------|------|------|-------------|-----------|-------------------|-------------|--------|
| 06-01-01 | 01 | 0 | V2-01 | registry/runtime | `docker compose run --rm api pytest -q tests/test_config_and_registry.py -k "panel or policy or implemented"` | ✅ | ⬜ pending |
| 06-01-02 | 01 | 0 | V2-01 | service + interface skip semantics | `docker compose run --rm api pytest -q tests/test_analysis_flow.py tests/test_run_lifecycle.py tests/test_api.py tests/test_cli.py -k "skip or unsupported or panel or policy"` | ✅ | ⬜ pending |
...
| 06-03-03 | 03 | 2 | V2-01 | lifecycle integration | `docker compose run --rm api pytest -q tests/test_analysis_flow.py tests/test_run_lifecycle.py -k "market or macro or regulatory or skip"` | ✅ | ⬜ pending |
| 06-04-01 | 04 | 3 | V2-01 | prompt/registry inventory | `docker compose run --rm api pytest -q tests/test_config_and_registry.py tests/test_prompt_assets.py -k "expectation or catalyst"` | ✅ | ⬜ pending |
| 06-04-02 | 04 | 3 | V2-01 | support + skip semantics | `docker compose run --rm api pytest -q tests/test_analysis_flow.py tests/test_run_lifecycle.py -k "expectation or catalyst or skip"` | ✅ | ⬜ pending |
| 06-04-03 | 04 | 3 | V2-01 | rerun/delta + generated artifacts | `docker compose run --rm api pytest -q tests/test_monitoring_semantics.py tests/test_analysis_flow.py tests/test_run_lifecycle.py -k "expectation or catalyst or delta or rerun" && docker compose run --rm api pytest -q tests/test_generated_examples.py -k "expectation or catalyst or delta"` | ❌ | ⬜ pending |
| 06-05-01 | 05 | 4 | V2-01 | prompt/registry inventory | `docker compose run --rm api pytest -q tests/test_config_and_registry.py tests/test_prompt_assets.py -k "overlay or portfolio"` | ✅ | ⬜ pending |
| 06-05-02 | 05 | 4 | V2-01 | overlay support rules | `docker compose run --rm api pytest -q tests/test_analysis_flow.py tests/test_run_lifecycle.py tests/test_monitoring_semantics.py -k "overlay or portfolio or skip"` | ✅ | ⬜ pending |
| 06-05-03 | 05 | 4 | V2-01 | api/cli recommendation surfacing | `docker compose run --rm api pytest -q tests/test_api.py tests/test_cli.py -k "overlay or portfolio or recommendation"` | ✅ | ⬜ pending |
| 06-05-CP1 | 05 | 4 | V2-01 | checkpoint | Blocking `checkpoint:human-verify` in the final overlay plan | ✅ | ⬜ pending |
| 06-06-01 | 06 | 4 | V2-01 | docs contract | `docker compose run --rm api python -c "from pathlib import Path; files = [Path('README.md'), Path('docs/architecture.md'), Path('docs/runbook.md'), Path('docs/factor_ontology.md')]; text = '\\n'.join(path.read_text(encoding='utf-8').lower() for path in files); required = ['supply_product_operations', 'market_structure_growth', 'expectations_catalyst_realization', 'security_or_deal_overlay', 'portfolio_fit_positioning', 'skip', 'weak confidence', 'overall recommendation']; missing = [item for item in required if item not in text]; assert not missing, missing"` | ✅ | ⬜ pending |
| 06-06-02 | 06 | 4 | V2-01 | generated examples | `docker compose run --rm api pytest -q tests/test_generated_examples.py` | ✅ | ⬜ pending |
| 06-06-03 | 06 | 4 | V2-01 | final verification artifact | `docker compose run --rm api python -c "from pathlib import Path; text = Path('.planning/phases/06-productionize-remaining-panels/06-VERIFICATION.md').read_text(encoding='utf-8').lower(); required = ['v2-01', 'supply_product_operations', 'market_structure_growth', 'expectations_catalyst_realization', 'security_or_deal_overlay', 'portfolio_fit_positioning', 'skip', 'overall recommendation']; missing = [item for item in required if item not in text]; assert not missing, missing"` | ❌ | ⬜ pending |
