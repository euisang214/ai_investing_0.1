...
class AnthropicModelProvider(ModelProvider):
    ...
    def generate_structured_with_usage(
        ...
        usage = getattr(raw, "response_metadata", {}).get("usage", {})
        input_tokens = getattr(usage, "input_tokens", usage.get("input_tokens", 0)) if hasattr(usage, "input_tokens") or isinstance(usage, dict) else 0
        output_tokens = getattr(usage, "output_tokens", usage.get("output_tokens", 0)) if hasattr(usage, "output_tokens") or isinstance(usage, dict) else 0
